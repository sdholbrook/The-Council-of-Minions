...
<!-- ringer-check: export DOTNET_ROOT=$HOME/.dotnet; export PATH=$HOME/.dotnet:$HOME/.dotnet/tools:$HOME/.local/bin:$PATH; pwsh -File _bmad-output/implementation-artifacts/handoff-drafts-slice-validate.ps1 -->
<!-- ringer-owned: _bmad-output/implementation-artifacts/handoff-drafts-slice.json;_bmad-output/implementation-artifacts/handoff-drafts-slice-validate.ps1 -->
<!-- ringer-expect: _bmad-output/implementation-artifacts/handoff-drafts-slice.json;_bmad-output/implementation-artifacts/handoff-drafts-slice-validate.ps1 -->
...
## Story goal (FR12, FR13, FR15)

Drafts are prepared but NEVER sent without approval — draft-only is a hard
boundary.

## Slice must prove

1. One internal handoff draft and one external reply draft, each linked to a
   Work Item (sibling-slice CWI id) and source evidence, each marked
   `draftOnly: true` with `outboundApproved: false`.
2. An unapproved-outbound scenario: guard proving no outbound action occurred
   (`noOutboundWithoutApproval: true`) plus a policy-denial-style receipt for
   an attempted send without approval (reusing the 2-6 failure vocabulary).
3. One approved-outbound scenario: explicit approval receipt (scope = the one
   draft), after which the draft is marked releasable — approval authorizes
   ONLY that draft (a second draft stays draft-only).

## Validator must assert

Draft linkage resolves; draft-only + approval flags strict-bool and mutually
consistent (releasable requires an approval receipt whose scope names that
draft id); the denial receipt carries the 2-6 field set; the second draft
remains unreleased; receipts complete; cross-slice id uniqueness.

## Deliverables (exactly two files)

- `_bmad-output/implementation-artifacts/handoff-drafts-slice.json` (storyKey: `3-3-prepare-internal-handoff-and-external-reply-drafts`)
- `_bmad-output/implementation-artifacts/handoff-drafts-slice-validate.ps1` — prints
  `HANDOFF_DRAFTS_SLICE_VALIDATE_OK` and exits 0 on success; lists every issue and exits 1 otherwise. A
