...
4. Prove the app home shell renders the expected Council navigation groups instead of stopping at `Loading...`.
5. Drive the named curated view IDs for the MVP queue: `New Source Records`, `Proposed Work Items`, state-specific Work Item views, and `Recent Receipts`.
6. Open seeded record forms for the Source Record, Work Item, Receipt, and Minion Brief so form rendering fails red instead of hiding behind list-view success.
...
The current gate is intentionally text-marker based because Power Apps model-driven app DOM selectors are platform-owned and brittle. Missing business markers fail the run even if navigation succeeds. The runner also treats the current URL and page title as part of the error surface, so Dynamics error pages fail immediately even when the body text still says `Loading...`.

The runner must not use fixed sleeps for screen readiness. It waits for observable page markers, app error markers, and the requested environment URL. This keeps red/green feedback tied to the rendered Microsoft app instead of timer luck.

Current curation evidence: `app-curation-evidence.json` proves the app now pins 12 main forms, 30 views, and 18 manifest-curated views, with `ValidateApp` returning zero issues. The screen launcher consumes that evidence to navigate the actual curated views by ID. Final MVP acceptance still requires the screen gate and tenant evidence to stay current after any further app changes.
